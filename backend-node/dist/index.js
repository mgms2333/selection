...
// ============================================
// 获取工作站类型列表
app.get('/api/workstation-types', async (req, res) => {
    try {
...
// 获取指定工作站的选型步骤
app.get('/api/workstation/:id/steps', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)(`
      SELECT 
...
// 获取指定步骤的选项（支持联动过滤）
app.get('/api/steps/:id/options', async (req, res) => {
    try {
        const { id } = req.params;
        const { session_id } = req.query;
        // 获取该步骤的所有选项
        const optionsResult = await (0, db_1.query)(`
      SELECT 
    ...
    `, [id]);
      ...
      `, [id]);
            ...
            if (constraints.length > 0) {
                // 获取会话中之前步骤的选择
                const selectionsResult = await (0, db_1.query)(`
                ...
                for (const constraint of constraints) {
                    if (constraint.constraint_type === 'filter' && constraint.filter_values) {
                        ...
                        if (depSelection) {
                            ...
                            if (filterCategory) {
                                // 获取选中机械臂的品牌信息
                                const armResult = await (0, db_1.query)(`
...
// 保存当前步骤选择
app.post('/api/session/:id/select', async (req, res) => {
    try {
        const { id } = req.params;
...
// 获取会话当前步骤的选择
app.get('/api/session/:id/selections', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)(`
      SELECT 
